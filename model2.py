...
wandb.init(project = "DLProject", name = f"model2-lr-{os.environ['LEARNING_RATE']}-ep-{os.environ['EPOCHS']}-bs-{os.environ['BATCH_SIZE']}", entity = "02456_project")
wandb.config = {
...
LOAD_MODEL = False
LOADPATH = f"model_2_hpc_ba{BATCH_SIZE}-lr{LEARNING_RATE}-ep{NUM_EPOCHS}.pth.tar"
SAVEPATH = f"model_2_hpc_ba{BATCH_SIZE}-lr{LEARNING_RATE}-ep{NUM_EPOCHS}.pth.tar"
LOSS_PATH = f"model_2_loss_array_ba{BATCH_SIZE}-lr{LEARNING_RATE}-ep{NUM_EPOCHS}.npy"
...
#    model = nn.DataParallel(model)
    ...
    #        wandb.watch(model)
        ...
        if epoch == 30:
            ...
            }, f"model_2_hpc_ba{BATCH_SIZE}-lr{LEARNING_RATE}-ep30of{NUM_EPOCHS}.pth.tar")
        if epoch == 60:
            torch.save({
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
                ...
                'total_loss': total_loss,
                'loss_pr_epoch': loss_pr_epoch
            }, f"model_2_hpc_ba{BATCH_SIZE}-lr{LEARNING_RATE}-ep60of{NUM_EPOCHS}.pth.tar")
